...
class NewSettingsBase {
    public newGet<T extends SettingValue = SettingValue>(
        _id: string | RegExp,
    ...
    public newGet<T extends SettingValue = SettingValue>(
        _id: RegExp,
    ): SettingComposedValue<T>[];
    public newGet<T extends SettingValue = SettingValue>(
        _id: string | RegExp,
        callback?: SettingCallback,
    ): T | undefined | SettingComposedValue<T>[];
    public newGet<T extends SettingValue = SettingValue>(
        _id: string | RegExp,
        callback?: SettingCallback,
    ): T | undefined | SettingComposedValue<T>[] {
        if (callback !== undefined) {
            ...
            return undefined ; // Ok, needed for exhaustiveness check
        }
...
class NewAgenda {
    ...
    public async newEvery(
        interval: string | number,
        name: string,
        data: IJob['data'],
        options: RepeatOptions): Promise<Job>;
    public async newEvery(
        interval: string | number,
        name: string[],
        data: IJob['data'],
        options: RepeatOptions): Promise<Job[] | undefined>;
    public async newEvery(
        interval: string | number,
        name: string | string[],
        data: IJob['data'],
        options: RepeatOptions): Promise<Job | Job[] | undefined>;
    public async newEvery(
        interval: string | number,
        name: string | string[],
        data: IJob['data'],
