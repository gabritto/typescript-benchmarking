...
class NewKatex {
    ...
    renderMessage(message) {
        if (typeof message === 'string') {
            return /** @type {any} */(this.render(message)); // Ok
        }

        if (!message.html?.trim()) {
            return /** @type {any} */(message); // Ok
        }
...
function createKatexMessageRendering(options, _isMessage) {
    ...
    if (_isMessage) {
        return /** @type {any} */((/** @type {IMessage} */ message) => instance.renderMessage(message)); // Ok
    }
    return /** @type {any} */((/** @type {string} */ message) => instance.renderMessage(message)); // Ok
}
...
class NewSettingsBase {
    ...
    newGet(_id, callback) {
        if (callback !== undefined) {
            if (!Meteor.settings) {
                return /** @type {any} */(undefined); // Ok
            }
            if (_id === '*') {
                return /** @type {any} */(Object.keys(Meteor.settings).forEach((key) => {
            ...
            return /** @type {any} */(undefined); // Ok, needed for exhaustiveness check
        }

        if (!Meteor.settings) {
            return /** @type {any} */(undefined); // Error
        }

        if (_.isRegExp(_id)) {
            return /** @type {any} */(Object.keys(Meteor.settings).reduce((/** @type {SettingComposedValue<T>[]} */ items, key) => {
                const value = Meteor.settings[key];
                if (_id.test(key)) {
        ...
        return /** @type {any} */(Meteor.settings?.[_id]); // Error
    }
}
...
function getWithBug(group) {
    if (!group) {
        return /** @type {any} */(/** @type {Record<string, MessageBoxAction[]>} */({})); // Error
    }
    return /** @type {any} */(/** @type {MessageBoxAction[]} */([])); // Ok
}
...
function getWithoutBug(group) {
    if (group === undefined) {
        return /** @type {any} */(/** @type {Record<string, MessageBoxAction[]>} */({})); // Ok
    }
    return /** @type {any} */(/** @type {MessageBoxAction[]} */([])); // Ok
}
...
class NewAgenda {
    ...
    async newEvery(interval, name, data, options) {
        if (typeof name === 'string') {
            return /** @type {any} */(this._createIntervalJob(interval, name, data, options)); // Ok
        }

        if (Array.isArray(name)) {
            return /** @type {any} */(this._createIntervalJobs(interval, name, data, options)); // Ok
        }
...
function transform1(value) {
    if (value == null) return /** @type {any} */(null); // Ok
    if (typeof value !== 'string') {
        throw new Error();
    }
    return /** @type {any} */(value.toLowerCase()); // Ok
}
