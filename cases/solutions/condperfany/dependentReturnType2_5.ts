...
class NewKatex {
    ...
    renderMessage<T extends string | IMessage>(message: T):
        ...
        if (typeof message === 'string') {
            return this.render(message) as any; // Ok
        }

        if (!message.html?.trim()) {
            return message as any; // Ok
        }
...
export function createKatexMessageRendering<T extends true | false>(
    ...
    if (_isMessage) {
        return ((message: IMessage): IMessage => instance.renderMessage(message)) as any; // Ok
    }
    return ((message: string): string => instance.renderMessage(message)) as any; // Ok
}
    ...
    public newGet<C extends SettingCallback | undefined, I extends string | RegExp, T extends SettingValue = SettingValue>(
        ...
        if (callback !== undefined) {
            ...
            if (typeof _id === 'string') {
                ...
                return undefined as any; // Ok
            }

            return undefined as any; // Ok, needed for exhaustiveness check
        }
        ...
        return Meteor.settings?.[_id] as any; // Error
...
HelperCond<T, string, MessageBoxAction[], undefined, Record<string, MessageBoxAction[]>> {
    if (!group) {
        return {} as Record<string, MessageBoxAction[]> as any; // Error, could fall into this branch when group is empty string
    }

    return [] as MessageBoxAction[] as any; // Ok
}

function getWithoutBug<T extends string | undefined>(group: T):
HelperCond<T, string, MessageBoxAction[], undefined, Record<string, MessageBoxAction[]>> {
    if (group === undefined) {
        return {} as Record<string, MessageBoxAction[]> as any; // Ok
    }
...
class NewAgenda {
    ...
    public async newEvery<T extends string | string[]>(
        ...
        if (typeof name === 'string') {
            return this._createIntervalJob(interval, name, data, options) as any; // Ok
        }

        if (Array.isArray(name)) {
            return this._createIntervalJobs(interval, name, data, options) as any; // Ok
...
function transform1<T extends string | null | undefined>(value: T): HelperCond<T, string, string, null | undefined, null> {
    if (value == null) return null as any; // Ok
    if (typeof value !== 'string') {
        throw new Error();
    }
    return value.toLowerCase() as any; // Ok
}